...
namespace TTMDotNetCore.Shared;
...
public class AdoDotNetService
{
    ...
    public List<T> Query<T>(string query, params AdoDotNetParameter[]? parameters)
    {
        ...
        if(parameters is not null && parameters.Length > 0)
        {
            ...
            // cmd.Parameters.AddRange(parameters.Select(parameter => new SqlParameter(parameter.Name, parameter.Value)).ToArray());

            var parametersArray = parameters.Select(parameter => new SqlParameter(parameter.Name, parameter.Value)).ToArray();
